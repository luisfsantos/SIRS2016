...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    price = serializers.DecimalField(source='menu_item.price', decimal_places=2, max_digits=7, required=False,
                                     read_only=True)
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ('identifier', 'order_items', 'price', 'table_id', 'payment_method')
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            if validated_data['payment_method'] == 'CA':
                order.cash = 'FE'
